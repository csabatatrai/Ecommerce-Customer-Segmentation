...
## 7. Adatbetöltés, Time-Split és Célváltozó (Churn) kialakítása

### 7.1 Importok és konfiguráció
...
### 7.2 Adatbetöltés és Time-Split
...
### 7.3 RFM és kiterjesztett feature-ök kiszámítása
...
```python
# ============================================================
# 7.3 – RFM + kiterjesztett feature-ök kiszámítása (CSAK az X ablakból)
# ============================================================
...
### 7.4 Churn célváltozó (y) kialakítása
...
### 7.5 Feature mátrix (X) és célváltozó (y) elkülönítése
...
### 7.6 Holdout teszt szett elkülönítése
...
> ⚠️ **Fontos:** A CV a 9.1-es cellától kizárólag `X_train`-en fut. Az `X_test` az egész modellezési folyamat során „zár alatt marad", és csak a 9.4-es cellában nyitjuk ki egyetlen alkalommal a végső kiértékeléshez.
...
## 8. A/B Modellezés: Pipeline-ok felépítése

### 8.1 Az adatszivárgás-mentes K-Means Pipeline logikája
...
### 8.2 KMeansFeaturizer – egyedi, leakage-mentes transformer
...
```python
# ============================================================
# 8.1 – Egyedi Transformer: KMeans → OHE (Pipeline-kompatibilis)
# ============================================================
...
### 8.3 Modell A Pipeline – Csak RFM
...
```python
# ============================================================
# 8.2 – Modell A: Csak RFM Pipeline
# ============================================================
...
### 8.4 Modell B Pipeline – RFM + K-Means OHE (FeatureUnion)
...
```python
# ============================================================
# 8.3 – Modell B: RFM + K-Means Pipeline (FeatureUnion)
# ============================================================
...
## 9. Keresztvalidáció és modellek összehasonlítása
...
### 9.1 Keresztvalidáció mindkét modellre


```python
# ============================================================
# 9.1 – Keresztvalidáció mindkét modellre (CSAK X_train-en)
# ============================================================
...
### 9.2 Eredmények összehasonlítása
...
> **🔍 Modellválasztás értelmezése – Occam borotvája itt is érvényesül...**
>
...
### 9.3 CV eredmények vizualizálása
...
![png](images/03_churn_prediction/03_9.3_CV_eredmények_vizualizálása.png)
    


### 9.4 A nyertes modell betanítása és kiértékelése (Train vs. Teszt)
...
A tréning metrikák (X_train-en mért eredmények) természetesen optimistábbak lesznek – ez normális. Az igazi kérdés az, hogy **mekkora a rés** a tréning és a teszt teljesítmény között. Ezt a következő cellában (9.5 Overfitting Monitoring) vizsgáljuk meg részletesen.


```python
# ============================================================
# 9.4 – A nyertes modell betanítása és kiértékelése
# ============================================================
...
### 9.6 Hiperparaméter-hangolás `RandomizedSearchCV` segítségével *(opcionális)*
...
> 📌 **Megjegyzés az overfitting monitoringhoz:** A 9.5-ös táblázat az RSCV **előtti** állapotot rögzíti, ez szándékos pillanatkép, amely megmutatja, miért indokolt a hangolás. Ha az RSCV lefut és jobb modellt talál, a végleges Train−Teszt rés a **10.2-es összefoglalóban** olvasható, amely már a hangolt modell értékeit tükrözi.


```python
# ============================================================
# 9.6 Hiperparaméter-hangolás RandomizedSearchCV-vel (opcionális)
# ============================================================
# OPCIONÁLIS CELLA – a notebook többi része a 9.4-ben betanított
...
## 10. Végleges modell exportja

### 10.1 Végleges modell betanítása és mentése
...
```python
# ============================================================
# 10.1 – Végleges modell betanítása a TELJES adathalmazon és mentése
# ============================================================
import pickle
import types
...
### 10.2 Holdout teszt szett mentése a 04-es notebookhoz
